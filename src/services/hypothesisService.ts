...
const generateAndScoreHypotheses = async (
  ...
  const noveltyThreshold = generation_params?.novelty_threshold ?? 0.7; // Use ?? for default if 0 is a valid threshold
  ...
  const parsedHypothesesTexts = internalHelpers.parseHypothesesFromLLMResponse(llmResponseText, maxHypotheses, effectiveLogger);
  if (parsedHypothesesTexts.length === 0) {
  ...
  for (let i = 0; i < parsedHypothesesTexts.length; i++) {
    const text = parsedHypothesesTexts[i];
...
class HypothesisService {
  ...
  public async generateAndScoreHypotheses(
    ...
    const rawHypothesesTexts = this._parseHypothesesFromLLMResponse(llmResponseText, maxHypotheses);
    ...
    const noveltyThreshold = input.generation_params?.novelty_threshold ?? 0.5;
